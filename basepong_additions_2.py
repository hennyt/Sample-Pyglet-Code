...
class ForwardPaddle(Paddle):
    ...
    def deflect_ball(self, ball, side_hit):
        if self.player == 1 and side_hit == 'LEFT':
            # pass it through
            pass  # pass does nothing, but you need some command inside the if block to avoid an error
        elif self.player == 2 and side_hit == 'RIGHT':
            pass
        else:
            Paddle.deflect_ball(self, ball, side_hit)

    # Fancy version to work with curved paddles we did in lecture
    # def deflect_ball(self, ball, side_hit):
    #     if self.player == 1 and side_hit == 'LEFT':
    #         # pass it through but deflect according to position of intersection
    #         Paddle.deflect_ball(self, ball, 'RIGHT')
    #     elif self.player == 2 and side_hit == 'RIGHT':
    #         Paddle.deflect_ball(self, ball, 'LEFT')
    #     else:
